...
import {generarpdf} from '../../../UtileriasJS/GeneradorPdf';
...
async llenarFicha()
{
  let dataPDF = {
    ...
    //this.inmueble = {...resp}
    dataPDF.inmueble = {...resp}

    let respTipo= await this.getDescripcion(dataPDF.inmueble.TipoInmueble)
    //this.tipo = respTipo.Descripcion
    dataPDF.tipo = respTipo.Descripcion
  
    let respEstatus = await this.getDescripcion(dataPDF.inmueble.estatusinm)
    //this.estatus = respEstatus.Descripcion
    dataPDF.estatus = respEstatus.Descripcion
  
    const {imgresp}  = await this.GetImagenes(this.id)
    //this.imagenes = imgresp //{...imgresp}
    dataPDF.imagenes = imgresp
    //if (this.inmueble.IdInmueble) {
    if (dataPDF.inmueble.IdInmueble) {
    let objpdf=generarpdf(dataPDF)
